...
class Trader:
    ...
    def calculate_sharpe_ratio(self, n_years, n_days, ret):
        ...
        """
        rf = {2009: 0.0012, 2010: 0.0018, 2011: 0.0007,
              2012: 0.00024, 2013: 0.00027,
              2014: 0.00033, 2015: 0.00053, 2016: 0.0032, 2017: 0.0093, 2018: 0.0194}
    ...
    def calculate_portfolio_sharpe_ratio(self, performance, pairs, verbose):
        """
        ...
        rf = {2009: 0.0012, 2010: 0.0018, 2011: 0.0007,
              2012: 0.00024, 2013: 0.00027, 2014: 0.00033,
              2015: 0.00053, 2016: 0.0032, 2017: 0.0093, 2018: 0.0194}
...
#         if year in rf.keys():
        ...
        sharpe_ratio_iid = (annualized_ret - rf[year]) / (vol*np.sqrt(252))
        auto_corr = total_account_balance.pct_change().fillna(0).autocorr(lag=1)
        if verbose:
            print('Sharpe Ratio assumming IID returns: ',sharpe_ratio_iid)
        ...
        daily_sharpe_ratio = (annualized_ret-rf_daily) /vol
        if verbose:
...
#             sharpe_ratio = annualized_ret / (vol*np.sqrt(252))
        ...
        return sharpe_ratio_iid, auto_corr, daily_sharpe_ratio, vol

    def calculate_maximum_drawdown(self, account_balance, verbose=True):
        """
    ...
    def calculate_metrics(self, cum_returns, n_years, verbose=True):
        """
    ...
    def summarize_results(self, sharpe_results, cum_returns, performance, total_pairs, ticker_segment_dict, n_years, verbose=True):
        """
        ...
        avg_total_roi, avg_annual_roi, positive_pct = self.calculate_metrics(cum_returns, n_years, verbose)

        sharpe_ratio_iid, auto_corr, daily_sharpe_ratio, portfolio_vol = self.calculate_portfolio_sharpe_ratio(performance, total_pairs, verbose)
        ...
        max_dd, max_dd_duration, total_dd_duration = self.calculate_maximum_drawdown(total_account_balance, verbose)
        
        if verbose:
            print('Maximum drawdown of portfolio: {:.2f}%'.format(max_dd))

        results = {'n_pairs': len(sharpe_results),
                   'annual_sharpe_ratio_iid': sharpe_ratio_iid,
                   'auto_corr': auto_corr,
                   ...
                   'total_trades': pairs_df.positive_trades.sum() + pairs_df.negative_trades.sum(),
                   'positive_trades': pairs_df.positive_trades.sum(),
                   'negative_trades': pairs_df.negative_trades.sum(),
                   'pct_positive_trades_per_pair': avg_positive_trades_per_pair_pct,
                   'pct_pairs_with_positive_results': positive_pct,
                   
                   'max_dd': max_dd,
                   'max_dd_duration': max_dd_duration,
                   'total_dd_duration': total_dd_duration,
                   
                   'avg_half_life': pairs_df['half_life'].mean(),
                   'avg_hurst_exponent': pairs_df['hurst_exponent'].mean()}
        
        results.update(postfix_keys_to_dict(dict((pairs_df['positive_trades'] / len(pairs_df) ).describe()), 'positive_trades_dist'))
        results.update(postfix_keys_to_dict(dict((pairs_df['negative_trades'] / len(pairs_df) ).describe()), 'negative_trades_dist'))
        
        return results, pairs_df
  
    
    def get_results(self, results_with_costs, threshold_test, pairs, ticker_segment_dict, n_years_val, period_sample):
        sharpe_results_threshold_train_w_costs, cum_returns_threshold_train_w_costs = results_with_costs

        results, pairs_summary = self.summarize_results(sharpe_results_threshold_train_w_costs,
        ...
        dict_results = postfix_keys_to_dict(results, period_sample)
        dict_results.update( postfix_keys_to_dict(dict(pd.Series(sharpe_results_threshold_train_w_costs).describe()), 'pairs_sharpe_dist_'+ period_sample) )
